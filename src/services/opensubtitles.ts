...
const originalModule = new OpenSubtitles(
  process.env.OS_API_USERAGENT || "TemporaryUserAgent"
);
const osAPI = _.cloneDeep(originalModule);

osAPI.identify = async function (
  osQuery: OpenSubtitlesQuery
): Promise<OpensubtitlesIdentifyResponse> {
  try {
    return await originalModule.identify(osQuery);
  } catch (err) {
    if (
      err.message === "API seems offline" ||
      err.message.includes("Server under maintenance")
    ) {
